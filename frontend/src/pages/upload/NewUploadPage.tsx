      ...
      <CardContent className="grid max-w-md gap-3">
        <div className="grid gap-1.5">
          ...
          <Select value={trackerId ? String(trackerId) : ''} onValueChange={(v) => setTrackerId(Number(v))}>
            <SelectTrigger>
              <SelectValue placeholder={t('upload.chooseTracker')}>
                {(v: string | null) =>
                  selectLabel(trackers, v, (tr) => String(tr.id), (tr) => tr.label, t('upload.chooseTracker'))
                }
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
        ...
        {diskId !== null && (
          <div className="grid gap-1.5">
            <Label>{t('upload.sourceFile')}</Label>
            <Button variant="outline" onClick={() => setBrowserOpen(true)}>
              {relativePath || t('upload.chooseFile')}
            </Button>
            <DiskBrowserDialog
              diskId={diskId}
              open={browserOpen}
              ...
              title={t('upload.chooseFileToUpload')}
              mode="file"
            />
          </div>
        )}
...
function DraftStep({ jobId }: { jobId: number }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t('upload.contentIdentification')}</CardTitle>
        <CardDescription>
          {job.tmdb_id != null
            ? t('upload.autoResolved', { id: job.tmdb_id })
            : t('upload.notAutoResolved')}
        </CardDescription>
      </CardHeader>
        ...
        <Button
          ...
          onClick={() =>
            prepare.mutate(jobId, {
              onSuccess: () => toast.success(t('upload.prepareCompleted')),
              onError: (error) => toast.error(t('upload.prepareFailed', { message: error.message })),
            })
          }
        >
          {prepare.isPending ? t('upload.preparingInProgress') : t('upload.prepare')}
        </Button>
      </CardContent>
    </Card>
  )
...
function ReadyStep({ jobId }: { jobId: number }) {
  ...
  return (
    <div className="grid gap-6">
      <Card>
        <CardHeader>
          <CardTitle>{t('upload.resolvedFields')}</CardTitle>
          <CardDescription>
            {t('upload.typeIdGuessHint')}
          </CardDescription>
        </CardHeader>
        <CardContent className="grid max-w-2xl grid-cols-4 gap-3">
          ...
          <div className="grid gap-1.5">
            <Label>{t('upload.resolution')}</Label>
            <Select
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('upload.description')}</CardTitle>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t('upload.dupeCheckHint')}</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-2">
          <Button variant="outline" className="w-fit" onClick={() => dupeCheck.refetch()} disabled={dupeCheck.isFetching}>
            {dupeCheck.isFetching ? t('upload.checkingInProgress') : t('upload.checkDuplicates')}
          </Button>
          {dupeCheck.isError && (
            <p className="text-sm text-destructive">{dupeCheck.error.message}</p>
          )}
          {dupeCheck.data && dupeCheck.data.length === 0 && (
            <p className="text-sm text-muted-foreground">{t('upload.noDuplicatesFound')}</p>
          )}
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t('upload.publish')}</CardTitle>
          <CardDescription>{t('upload.humanConfirmationRequired')}</CardDescription>
        </CardHeader>
        <CardContent className="flex items-center gap-2">
          <Select value={torrentClientId ? String(torrentClientId) : ''} onValueChange={(v) => setTorrentClientId(Number(v))}>
            <SelectTrigger className="w-56">
              <SelectValue placeholder={t('upload.seedingClientPlaceholder')}>
                {(v: string | null) =>
                  selectLabel(torrentClients, v, (tc) => String(tc.id), (tc) => tc.label, t('upload.seedingClientPlaceholder'))
                }
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
          ...
          <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
            <DialogTrigger render={<Button disabled={!torrentClientId}>{t('upload.confirmAndPublish')}</Button>} />
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{t('upload.confirmPublishTitle')}</DialogTitle>
                <DialogDescription>
                  {t('upload.confirmPublishDescription')}
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <Button
                  ...
                  onClick={() =>
                    ...
                    confirm.mutate(
                      { uploadId: jobId, torrentClientId },
                      {
                        ...
                        onError: (error) => toast.error(t('upload.publishFailed', { message: error.message })),
                      },
                    )
                  }
                >
                  {t('upload.yesPublishNow')}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
