"""OnlyImage — host immagini famiglia Chevereto, api_key gratuita. Shape
...
class OnlyimageAdapter(ImageHostAdapter):
    ...
    def upload(self, image_path: str) -> str:
        try:
            ...
            raise ImageHostError(f"Upload OnlyImage fallito: {exc}") from exc

        url = chevereto_image_url(data)
        if not url:
            raise ImageHostError(f"Risposta OnlyImage senza URL riconoscibile: {data!r}")
        return url
